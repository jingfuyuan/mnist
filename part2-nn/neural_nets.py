...
class NeuralNetwork():
    ...
    def train(self, x1, x2, y):
        ...
        output_layer_error = output_layer_error.item()
        # print(output_layer_error)
        vfunc = np.vectorize(rectified_linear_unit_derivative)
        hidden_layer_error =  output_layer_error * np.multiply(self.hidden_to_output_weights.T, \
            vfunc(hidden_layer_weighted_input)) # TODO (3 by 1 matrix)

        # print(hidden_layer_error)
        # print(self.hidden_to_output_weights.T)
        # print(vfunc(hidden_layer_weighted_input))
        ...
        input_to_hidden_weight_gradients = hidden_layer_error.dot(input_values.T) # TODO
